...
```bash
# 构建生产镜像
docker build -t whois-query-tool:latest .

# 构建开发镜像
docker build -f Dockerfile.dev -t whois-query-tool:dev .
```
...
docker run -d \
  ...
  whois-query-tool:latest
```

**开发环境运行：**
```bash
docker run -d \
  --name whois-query-tool-dev \
  -p 3000:3000 \
  -p 9229:9229 \
...
| `PORT` | `3000` | 应用监听端口 |
| `HOSTNAME` | `0.0.0.0` | 应用绑定地址 |
| `NEXT_TELEMETRY_DISABLED` | `1` | 禁用 Next.js 遥测 |
| `WATCHPACK_POLLING` | `false` | 开发环境文件监听（Docker 中建议启用） |

**自定义环境变量文件：**
```bash
# 创建 .env.local 文件
echo "NODE_ENV=production" > .env.local
echo "CUSTOM_API_KEY=your_api_key" >> .env.local
...
```bash
# 默认端口映射
-p 3000:3000          # HTTP 服务端口

# 开发环境额外端口
-p 9229:9229          # Node.js 调试端口

# 自定义端口映射
-p 8080:3000          # 将容器 3000 端口映射到主机 8080 端口
```
...
# 使用命名卷
docker volume create whois-logs
-v whois-logs:/app/logs
```
...
# 查看容器日志
docker logs whois-query-tool

# 实时查看日志
docker logs -f whois-query-tool

# 进入容器
docker exec -it whois-query-tool sh

# 停止容器
docker stop whois-query-tool

# 重启容器
docker restart whois-query-tool

# 删除容器
docker rm whois-query-tool

# 查看容器资源使用
docker stats whois-query-tool
```

#### 8. 健康检查

容器内置健康检查功能，可通过以下方式查看：

```bash
# 查看健康状态
docker inspect --format='{{.State.Health.Status}}' whois-query-tool

# 查看健康检查历史
docker inspect --format='{{range .State.Health.Log}}{{.Output}}{{end}}' whois-query-tool
```
...
- **Docker 部署**：[http://localhost:3000](http://localhost:3000)（或自定义端口）
...
本项目采用 GNU Affero General Public License v3.0（AGPLv3）许可。

- 强传染性：任何修改、分发或以网络形式提供本软件服务的衍生作品，必须在同一许可证下开源发布；
- 源码公开义务：当通过网络使用本软件（例如提供在线查询服务）时，您必须向服务使用者开放修改后的完整源代码及相关构建脚本；
- 版权与许可说明：保留原作者版权声明与许可文本，不得移除或弱化；
- 贡献协议：所有贡献默认以 AGPLv3 许可进行授权，合并后与项目保持一致的开源要求。

如需商业合作或双许可方案，请联系维护者以讨论合规路径与授权方式。
## 🙏 致谢
